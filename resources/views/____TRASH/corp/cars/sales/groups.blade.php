              ...
              <tbody> 
                ...
                @foreach ($groups as $group)
                  ...
                  <tr> 
                    ...
                    <td>
                      <p><strong>{{ $group->title }}</strong> <span class="label label-danger">group</span><span class="pull-right"><span class="label label-default">{{ $group->sales()->count() }} cars</span></span></p>
                      <p>{{ $group->descript }}</p>
                    </td> 
                    <td style="font-size:11px;color:grey">started {{ $group->startdate->diffForHumans() }}</td> 
                    <td style="font-size:15px;"><a href="{{ url('/corporate/'.$corporate->id.'/cars/sales/groups/group/'.$group->id) }}" class="label label-primary">view</a></td>
                  </tr> 
