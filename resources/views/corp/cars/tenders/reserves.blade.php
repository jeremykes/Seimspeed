    ...
    @allowed('corporate.user', $corporate)
      ...
      @role('sales|administrator')
        ...
        <div class="col-md-12">
            ...
            <span style="color:grey;font-size:10px">></span>
            <a style="color:grey;font-size:9px" class="btn btn-xs btn-default" href="{{ url('corporate/cars/tenders/reserves') }}">Reserved</a>
            ...
            <table class="table table-bordered table-striped" style="margin-bottom:0"> 
              <tbody> 
                ...
                @foreach ($reserves as $reserve)
                  ...
                  <tr> 
                    <td><img class="img-responsive" src="{{ $reserve->cartender->car->images()->first()->thumb_img_url }}"></td>
                    <td>

                      <p><strong>{{ $reserve->cartender->car->make.' '.$reserve->cartender->car->model }}</strong> reserved by <a href="#">{{ $reserve->cartendertender->user->name }}</a></p>
                      <p>{{ $reserve->note }}</p>
                      <p><span style="color:gray;font-size:11px">{{ $reserve->cartender->car->comments()->count() }}  <i class="fa fa-comment-o"></i></span>&nbsp;&nbsp;&nbsp;&nbsp;<span style="color:gray;font-size:11px">{{ $reserve->cartender->tenders()->count() }}  <i class="fa fa-money"></i></span>&nbsp;&nbsp;&nbsp;&nbsp;<span style="color:gray;font-size:11px">{{ $reserve->cartender->car->tails()->count() }}  <i class="fa fa-eye"></i></span></p>
                    </td> 
                    <td style="font-size:11px;color:grey">{{ $reserve->created_at->diffForHumans() }}</td> 
                    <td style="font-size:15px;"><a href="{{ url('corporate/'.$corporate->id.'/cars/tenders/tender/'.$reserve->cartender->id.'/reserves/car') }}" class="label label-primary">view</a></td>
                  </tr> 
